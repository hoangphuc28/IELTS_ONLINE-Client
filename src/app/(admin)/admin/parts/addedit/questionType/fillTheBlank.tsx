...
import { Answer, DragAndDrop, Dropdown, FillTheBlank, MultipleResponse } from '../../type/Question'
import {
    Accordion,
    AccordionActions,
    AccordionDetails,
...
export default function FillBlankForm(props: QuestionProps) {
    const DynamicEditor = dynamic(() => import('../_components/editor/editor'), { ssr: false })
    const Editor = useMemo(() => DynamicEditor, [])
    const [question, setQuestion] = useState<FillTheBlank>(props.data as FillTheBlank)
    return (
        <Fragment>
                    ...
                    <div className="flex justify-start items-start mt-5">
                        ...
                        <input
                            ...
                            onChange={(event: any) => {
                                ...
                                const subAnswers = newValue.split(/\s*,\s*/).filter((element: string) => element !== '');
                                updatedAnswers[i].content = newValue
                                updatedAnswers[i].subAnswers = subAnswers
